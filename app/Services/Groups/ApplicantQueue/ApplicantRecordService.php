...
{
    public function __construct(private readonly CharacterZoneProgressFetcher $fetcher) {}
    ...
    public function forApplication(Activity $activity, ActivityApplication $application): array
    {
        ...
        $history = $characterId ? $this->completedRuns($activity, $characterId) : collect();
        ...
        $rows = $definitions->map(function (array $definition) use ($history, $characterId, $encounters, $fflogsStatus, &$usedEncounters) {
            ...
            foreach ($history as $run) {
                ...
                $matches = $run->progressMilestones->filter(fn ($milestone) => $this->sameMilestone(
                    $definition,
    ...
    private function completedRuns(Activity $activity, int $characterId): Collection
    {
        ...
        return Activity::query()
            ...
            ->get(['id', 'activity_type_version_id']);
    }

    private function sameMilestone(array $current, array $historical): bool
    {
