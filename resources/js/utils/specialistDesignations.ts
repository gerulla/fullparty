...
export function specialistDesignationMarkers(slot: ActivitySlot, hasRightCornerMarker = false) {
    const positions = ['-right-2 -top-2', 'right-5 -top-2', 'right-12 -top-2'];
...
export function specialistDesignationActions(
    ...
    return definitions.filter(definition => slot.available_designations?.includes(definition.key)).map(definition => ({
        ...
        ui: definition.key === 'trapper' ? { itemLabel: 'text-orange-500', itemLeadingIcon: 'text-orange-500' } : undefined,
