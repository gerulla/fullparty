...
<template>
	...
	<div>
		<div class="mb-1 mx-auto">
			<p class="italic text-center text-gray-600 dark:text-gray-300">{{ t('pages.auth.express_options') }}</p>
		</div>
		...
		<div class="flex items-center w-full mb-4">
			<form class="space-y-4 w-full" @submit.prevent="submit">
				<UFormField name="username" class="w-full" :error="form.errors.username">
					<UInput v-model="form.username" size="xl" class="w-full" :placeholder="t('pages.auth.username')"/>
				</UFormField>

				<UFormField name="email" class="w-full" :error="form.errors.email">
					<UInput v-model="form.email" type="email" size="xl" class="w-full" :placeholder="t('pages.auth.email')"/>
				</UFormField>

				<UFormField name="password" :error="form.errors.password">
					<UInput v-model="form.password" type="password" size="xl" class="w-full" :placeholder="t('pages.auth.password')" />
				</UFormField>

				<UFormField name="password_confirmation">
					<UInput v-model="form.password_confirmation" type="password" size="xl" class="w-full" :placeholder="t('pages.auth.password_confirmation')" />
				</UFormField>
		...
		<div class="flex items-center justify-center flex-col space-y-2 w-full">
			<p>{{ t('pages.auth.existing_account') }} <Link href="/auth/login" class="text-brand">{{ t('pages.auth.log_in_now') }}</Link></p>
		</div>
	</div>
</template>
