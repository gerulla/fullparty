...
const actions = computed(() => resource.value ? [
    { label: l('move'), icon: 'i-lucide-folder-input', disabled: resource.value.status === 'pending', onSelect: () => props.workspace.openMove([resource.value!.id]) },
    { label: l('delete'), icon: 'i-lucide-trash-2', color: 'error' as const, onSelect: () => props.workspace.remove(resource.value!.id) },
] : [])
...
<template>
    <aside class="library-inspector">
        <template v-if="resource">
            <header class="inspector-heading"><h2>{{ resource.title }}</h2><UBadge :color="resource.status === 'published' ? 'success' : resource.status === 'pending' ? 'warning' : 'neutral'" variant="subtle" size="sm" class="inspector-status">{{ l(resource.status) }}</UBadge><UDropdownMenu :items="actions"><UButton icon="i-lucide-ellipsis" color="neutral" variant="ghost" size="xs" :aria-label="l('actions')" /></UDropdownMenu></header>
            <div class="inspector-overview">
                <button v-if="resource.cover" class="inspector-cover" :aria-label="l('preview')" @click="workspace.preview(resource)"><img :src="resource.cover" alt="" /><span class="cover-expand"><UIcon name="i-lucide-expand" /></span></button>
                <p class="resource-description">{{ resource.description }}</p>
                <div class="inspector-actions"><UButton v-if="resource.status !== 'pending'" icon="i-lucide-pencil" color="neutral" variant="solid" :label="l('edit_resource')" @click="workspace.edit(resource.id)" /><UButton v-else icon="i-lucide-check" :label="l('publish')" @click="workspace.publish([resource.id])" /><UButton icon="i-lucide-external-link" color="neutral" variant="outline" :label="l('preview')" @click="workspace.preview(resource)" /><UDropdownMenu :items="actions"><UButton icon="i-lucide-ellipsis" color="neutral" variant="outline" :aria-label="l('actions')" /></UDropdownMenu></div>
                <div v-if="resource.status === 'pending'" class="pending-revision"><p>{{ l('pending_lock') }}</p><UButton icon="i-lucide-trash-2" variant="solid" color="error" size="sm" class="mt-2" :label="l('discard_pending')" @click="workspace.discardPending(resource.id)" /></div>
                <dl class="inspector-metadata">
                    ...
                    <div><dt>{{ l('author') }}</dt><dd><img :src="'/characters/char1.png'" alt="" class="author-avatar" /><span>{{ resource.author }}</span></dd></div>
            ...
            <section class="inspector-discord">
                <header>
                    <h3>{{ l('discord_command') }}</h3>
                    <UButton v-if="resource.embed.enabled" icon="i-lucide-settings" color="neutral" variant="outline" size="sm" :label="l('configure')" :disabled="resource.status === 'pending'" @click="workspace.edit(resource.id, 'discord')" />
                </header>
                <template v-if="resource.embed.enabled">
                    <pre v-if="resource.embed.command" class="resource-embed-command"><code>/info {{ resource.embed.command }}</code></pre>
                    <ResourceEmbedPreview :document="resource" compact @open="workspace.preview(resource)" />
                </template>
