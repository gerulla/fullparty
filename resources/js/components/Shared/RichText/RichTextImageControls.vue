...
const groupAlign = ref<RichTextImageAlign>('center')
const layouts = computed(() => (['block', 'wrap-left', 'wrap-right', 'inline'] as const).map(value => ({ value, label: l(`image_${value.replace('-', '_')}`) })))

function sync() {
    const selection = props.editor.state.selection
    const group = selectedImageGroup(props.editor.state)
    grouped.value = group !== null
    groupAlign.value = group?.node.attrs.align ?? 'center'
...
<template>
    <div v-if="grouped" class="flex basis-full flex-wrap items-center gap-2 border-t border-default pt-2" role="group" :aria-label="l('image_group')">
        <span class="flex items-center gap-1 text-xs font-semibold"><UIcon name="i-lucide-images" />{{ l('image_group') }}</span>
        <div class="flex gap-1" role="group" :aria-label="l('image_group_alignment')">
            <UTooltip v-for="value in (['left', 'center', 'right'] as const)" :key="value" :text="l(value)">
                <UButton :icon="`i-lucide-align-${value}`" :color="groupAlign === value ? 'primary' : 'neutral'" :variant="groupAlign === value ? 'soft' : 'ghost'" size="xs" :aria-label="l(value)" :aria-pressed="groupAlign === value" @click="editor.chain().focus().setImageGroupAlign(value).run()" />
            </UTooltip>
        </div>
        <UButton icon="i-lucide-image-plus" :label="l('image_group_add')" color="neutral" variant="outline" size="xs" @click="emit('image')" />
        <UButton icon="i-lucide-ungroup" :label="l('image_group_ungroup')" color="neutral" variant="ghost" size="xs" @click="editor.chain().focus().ungroupImages().run()" />
        <span class="basis-full text-xs text-muted">{{ l('image_group_hint') }}</span>
    </div>
    <div v-if="selected" class="flex basis-full flex-wrap items-center gap-2 border-t border-default pt-2" role="group" :aria-label="l('image_options')">
        ...
        <USelect v-if="!grouped" :model-value="layout" :items="layouts" :aria-label="l('image_layout')" size="xs" class="w-44" @update:model-value="value => editor.chain().focus().setRichTextImageLayout(value).run()" />
        <div v-if="!grouped && layout === 'block'" class="flex gap-1" role="group" :aria-label="l('image_alignment')">
            <UTooltip v-for="value in (['left', 'center', 'right'] as const)" :key="value" :text="l(value)">
        ...
        <UButton v-if="!grouped && layout !== 'inline'" icon="i-lucide-group" :label="l('image_group_create')" color="neutral" variant="outline" size="xs" @click="editor.chain().focus().createImageGroup().run()" />
        ...
        <UTooltip :text="l('delete_image')"><UButton icon="i-lucide-trash-2" color="error" variant="soft" size="xs" :aria-label="l('delete_image')" @click="editor.chain().focus().deleteRichTextImage().run()" /></UTooltip>
