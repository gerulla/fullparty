...
<template>
	...
	<div>
		...
		<div class="flex items-center w-full mb-4">
			<form class="space-y-4 w-full" @submit.prevent="submit">
				<UFormField name="email" class="w-full" :error="form.errors.email">
					<UInput v-model="form.email" size="xl" class="w-full" :placeholder="t('pages.auth.email')"/>
				</UFormField>
				...
				<UCheckbox :label="t('pages.auth.remember_me')" name="remember" v-model="form.remember"/>
		...
		<div class="flex items-center justify-center flex-col space-y-2 w-full">
			<Link href="/auth/password" class="text-brand">{{ t('pages.auth.forgot_password') }}</Link>
			<p>{{ t('pages.auth.no_account') }} <Link href="/auth/register" class="text-brand">{{ t('pages.auth.sign_up_now') }}</Link></p>
		</div>
	</div>
</template>
