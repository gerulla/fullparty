...
class ResourceHolsterService
{
    ...
    public function configure(Group $group, User $user, array $input): array
    {
        ...
        DB::transaction(function () use ($group, $user, $input) {
            ...
            if ($library->holster_collection_id !== $data['collection_id']) {
                ...
                abort_if((clone $linked)->whereHas('holster', fn ($holster) => $holster->where('type', BozjaHolster::TYPE_PREPOP))
                    ->where('editing_expires_at', '>', now())->exists(), 409, __('resource_errors.locked'));
    ...
    public function presentation(GroupResource $resource, ?User $user, bool $public): array
    {
        ...
        $refills = $holster->refillHolsters()
            ->where('group_id', $group->id)->where('is_active', true)->whereNull('moderation_hidden_at')
            ->where('type', BozjaHolster::TYPE_REFILL)
            // Older installations have refill resource rows. Preserve any visibility
            // restrictions on those rows without requiring them for new refills.
            ->whereNotExists(fn ($legacy) => $legacy->selectRaw('1')->from('group_resources')
                ->whereColumn('group_resources.holster_id', 'bozja_holsters.id')
                ->where(fn ($hidden) => $hidden->whereNotNull('moderation_hidden_at')
                    ->orWhere('status', '!=', 'published')->orWhereNull('published_revision_id')
                    ->orWhereNotIn('access_level', $levels)))
    ...
    public function management(Group $group, ?User $user = null): array
    {
        ...
        return [
            ...
            'active_count' => BozjaHolster::where('group_id', $group->id)->where('is_active', true)->where('type', BozjaHolster::TYPE_PREPOP)->count(),
            // Linked holsters are already in workspace.resources. Keep the old key
            // empty so previously deployed managers do not render a second listing.
            'resources' => [],
        ];
    }
}
